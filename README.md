...
> - **断点续传与 `--dry-run` 的数据就绪判断**：仍会按股票所属市场的本地时区和交易日历解析“最新可复用交易日”；但在 Agent 模式下，`--dry-run` 和正式执行都会以“历史 K 线缓存已按目标交易日补齐到 Agent 基线深度（当前为 240 天）”作为成功标准，而不再仅看目标交易日是否有单日数据。详细规则见 [完整指南](docs/full-guide.md)。
