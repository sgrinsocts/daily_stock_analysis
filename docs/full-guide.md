...
- 断点续传和 `--dry-run` 都会先共用同一套“最新可复用交易日”解析逻辑，不再直接使用服务器自然日
...
- 非 Agent 路径的断点续传与 `--dry-run` 成功标准仍是“目标交易日数据已存在”
- Agent 路径会把“数据已就绪”收紧为“目标交易日对应的历史 K 线缓存已补齐到 Agent 基线深度（当前为 240 天）”；因此 Agent 模式下 `--dry-run` 的最终成功/失败汇总，和正式执行的 Step 1 数据准备结果保持一致
